...
namespace gr {
  namespace lora_sdr {
            ...
            gr::io_signature::make(1, 1, sizeof(uint8_t)))
    {
        ...
        message_port_register_out(pmt::mp("corrected"));
            ...
            switch(cr_app){
                case 4:
                    if (!(count(codeword.begin(), codeword.end(), true) % 2)){ //Don't correct if even number of errors
                        message_port_pub(pmt::intern("corrected"),pmt::mp((int)0));
                        message_port_pub(pmt::intern("detected"),pmt::mp((int)0));
                        break; // 0, 2 ou 4 erreurs -> non corrigees
                    }
                case 3:
                    //get syndrom
                    ...
                    switch(syndrom){
                        case 5:
                            data_nibble[3].flip();
                            message_port_pub(pmt::intern("corrected"),pmt::mp((int)1));
                            message_port_pub(pmt::intern("detected"),pmt::mp((int)1));
                            break;
                        case 7:
                            data_nibble[2].flip();
                            message_port_pub(pmt::intern("corrected"),pmt::mp((int)1));
